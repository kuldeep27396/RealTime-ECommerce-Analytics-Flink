...
class ProducerConfig(BaseSettings):
    ...
    API_DURATION: int = Field(
        default=60, ge=10, le=300, description="Duration in seconds for each API call"
    )
    ...
    # Producer Settings
    KAFKA_ACKS: str = Field(default="all", description="Producer acks setting")
    KAFKA_RETRIES: int = Field(
        default=3, ge=0, le=10, description="Number of retry attempts"
    )
    KAFKA_MAX_IN_FLIGHT: int = Field(
        default=1, ge=1, le=5, description="Maximum in-flight requests"
    )
    ...
    METRICS_PORT: int = Field(
        default=8080, ge=1024, le=65535, description="Metrics server port"
    )
    LOG_LEVEL: str = Field(default="INFO", description="Logging level")

    # Batch Settings
    BATCH_SIZE: int = Field(
        default=100, ge=1, le=1000, description="Batch size for producing messages"
    )
    BATCH_TIMEOUT_MS: int = Field(
        default=1000, ge=100, le=10000, description="Batch timeout in milliseconds"
    )
